...
layout = [
    [field],
    [sg.Text('Apples eaten:', font='Young 20', key='-APPLECOUNT-')],
    [sg.Text('You lose, closing the window in 5 seconds...', font='Young 20', text_color='red', visible=False, key='-LOSEMSG-')]
]

window = sg.Window('Snake Game', layout, return_keyboard_events=True, element_justification='center')
...
while True:
    ...
    if time_since_start >= 0.5:
        ...
        if snake_body[0] == apple_pos:
            apple_count += 1
            apple_count_str = apple_count.__str__()
            window['-APPLECOUNT-'].update('Apples eaten: ' + apple_count_str)
            apple_pos = place_apple()
            apple_eaten = True
        ...
        if not 0 <= snake_body[0][0] <= CELL_NUM - 1 or \
            ...
            window['-LOSEMSG-'].update(visible=True)
            sleep(5)
            break
